...
- `@dynamicMemberLookup` is _not_ a back door around access control. The
  outside caller never reads the private `y`; they read whatever the
  subscript returns.
- But it _is_ a name-collision hazard. Adding a `private var someName`
...
The same fallback fires for _any_ name the outside caller can't see:
