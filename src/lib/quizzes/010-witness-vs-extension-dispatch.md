...
- A method **declared in the protocol body** is a *requirement*. The
...
- A method **only declared in a protocol extension** is *not* a
...
| `q.g()` | `Q`         | no           | n/a — static dispatch picks the *extension* `Q.g` → "1" | `1` |

The fourth line is the trap. Even though `q`'s underlying value is `S`
(which has its own `g()`), `g` is *not* a protocol requirement, so the
